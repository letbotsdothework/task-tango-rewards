-- Fix infinite recursion in RLS policies (updated)
...
DROP POLICY IF EXISTS "Invited users can view their invite by email" ON public.household_invites;
...
-- Add policy for viewing own profile (if doesn't exist)
DROP POLICY IF EXISTS "Users can view their own profile always" ON public.profiles;
...
DROP POLICY IF EXISTS "Users with invites can view household names" ON public.households;
